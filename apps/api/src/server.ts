...
import { invokeAgentGraph, resumeAgentGraph, streamAgentGraphEvents, toLegacyChatResponse } from "@creator/agent-graph";
...
const writeDataChunk = (writer: { write: (part: UIMessageChunk) => void }, name: string, data: unknown, id: string = randomUUID()) => {
  writer.write({
    type: `data-${name}`,
    id,
    data
...
const writeToolEvent = (writer: { write: (part: UIMessageChunk) => void }, event: Extract<AgentStreamEvent, { type: "tool-call" }>) => {
  writer.write({
...
app.post("/api/chat/stream", async (request, reply) => {
  ...
  const stream = createUIMessageStream<UIMessage>({
    execute: async ({ writer }) => {
      ...
      for await (const event of streamAgentGraphEvents({ request: parsed.data, diagnosis })) {
        switch (event.type) {
          case "thread":
            finalMetadata = {
              ...finalMetadata,
          ...
          case "tool-call":
            ...
            writeDataChunk(writer, "agent-tool-call", event.toolCall, event.toolCall.id);
            break;
          case "agent-run-patch":
            writeDataChunk(writer, "agent-run-patch", event.patch satisfies AgentRunPatch, event.patch.id);
            break;
          case "text-delta":
            if (!textStarted) {
          ...
          case "agent-run":
            ...
            writeDataChunk(writer, "agent-run", event.agentRun, event.agentRun.id);
            break;
          case "approval-requested":
            writeDataChunk(writer, "agent-approval", event.approval, event.approval.id);
            break;
          case "finish":
            finalMetadata = {
              ...finalMetadata,
              ...
              usedModules: event.agentRun?.usedModules ?? finalMetadata.usedModules
            };
            writeDataChunk(writer, "agent-finish", event, event.threadId ?? randomUUID());
            break;
          case "error":

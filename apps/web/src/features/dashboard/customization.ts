...
export const dashboardBreakpoints: DashboardBreakpoint[] = [
  "lg",
  "md",
  "sm",
  "xs",
];
export const dashboardBoardColumns: DashboardBoardColumn[] = [
  "today",
  "next",
  "this_week",
  "done",
...
export const getDashboardBreakpointForWidth = (
  width: number,
): DashboardBreakpoint => {
...
export const dashboardCardSizeOrder: DashboardCardSize[] = [
  "small",
  "medium",
  "large",
];
...
const matchedSummaryHeightKinds = new Set<DashboardCardKind>([
  "summary",
  "insights",
]);
...
export const normalizeDashboardCardSize = (
  value: unknown,
  fallback: DashboardCardSize = "medium",
): DashboardCardSize => {
...
const defaultDimensionsForSize = (
...
const normalizeDashboardCardDimensionsWithFallback = (
  ...
  if (typeof value === "string") {
    return (
      legacyDimensionMap[value] ??
      defaultDimensionsForSize(normalizeDashboardCardSize(value, fallbackSize))
    );
  }
  ...
  const legacyDimensions =
    typeof preference.size === "string"
      ? (legacyDimensionMap[preference.size] ??
        defaultDimensionsForSize(
          normalizeDashboardCardSize(preference.size, fallbackSize),
        ))
      : undefined;

  return {
    width: normalizeDashboardCardSize(
      preference.width,
      legacyDimensions?.width ?? fallback.width,
    ),
    height: normalizeDashboardCardSize(
      preference.height,
      legacyDimensions?.height ?? fallback.height,
    ),
  };
};
...
const normalizeGridNumber = (value: number, fallback: number) =>
  Number.isFinite(value) ? Math.round(value) : fallback;

const normalizeDashboardColumnCount = (cols: number, fallback: number) =>
  Math.max(1, normalizeGridNumber(cols, fallback));
...
export const getDashboardCardGridConstraints = (
  ...
  const columnCount = normalizeDashboardColumnCount(
    cols,
    dashboardColumnCounts[breakpoint],
  );
...
export const normalizeDashboardGridItem = (
  ...
  const constraints = getDashboardCardGridConstraints(
    card,
    breakpoint,
    columnCount,
  );
  const w = clamp(
    normalizeGridNumber(item.w, constraints.minW),
    constraints.minW,
    constraints.maxW,
  );
  const x = clamp(
    normalizeGridNumber(item.x, 0),
    0,
    Math.max(columnCount - w, 0),
  );
  ...
  return {
    ...
    w,
    h: clamp(
      normalizeGridNumber(item.h, constraints.minH),
      constraints.minH,
      constraints.maxH,
    ),
    ...constraints,
...
export const getDashboardCardContentSizeForGridItem = (
  ...
  const columnCount = normalizeDashboardColumnCount(
    cols,
    dashboardColumnCounts.lg,
  );
  const isFullWidth = item.w >= columnCount;
  const keepsRichContentOnNarrowGrid =
    columnCount <= dashboardColumnCounts.xs &&
    isFullWidth &&
    card?.kind !== "metric";
  const keepsRichContentWhenFullWidth =
    ...
    (card?.kind === "summary" ||
      card?.kind === "insights" ||
      card?.kind === "module-chart");

  if (
    card?.kind === "summary" &&
    columnCount >= dashboardColumnCounts.md &&
    item.w >= 7 &&
    item.h >= 7
  ) {
    return "large";
  ...
  if (
    (keepsRichContentOnNarrowGrid || keepsRichContentWhenFullWidth) &&
    item.h >= Math.max(item.minH ?? 1, 6)
  ) {
    return item.h >= 10 ? "large" : "medium";
  }

  if (
    item.w <= Math.max(item.minW ?? 1, 3) ||
    item.h <= Math.max(item.minH ?? 1, 6)
  ) {
    return "small";
  }
...
export const createActionCardId = (insightId: string, index: number) =>
  `action:${insightId}:${index}`;
export const createModuleChartCardId = (moduleId: string) =>
  `module-chart:${moduleId}`;
export const isModuleChartCardId = (cardId: string) =>
  cardId.startsWith("module-chart:");

const commerceMetricCardId = "metric:commerce";
const revenueFocusCardIds = [
  commerceMetricCardId,
  createModuleChartCardId("drama-revenue-radar"),
  createModuleChartCardId("viral-review"),
] as const;

const hasRevenueFocusCards = (cards: DashboardCardDefinition[]) =>
  revenueFocusCardIds.every((cardId) =>
...
export const buildDashboardCards = (
  ...
  const cards: DashboardCardDefinition[] = [
    ...
    ...viewModel.metricCards.map((metric, index) => ({
      ...
      priority: 10 + index,
      defaultSize:
        metric.id === "commerce" ? ("medium" as const) : ("small" as const),
...
export const buildDashboardActionCards = (
  diagnosis: DiagnosisResponse,
): DashboardActionCard[] =>
...
const getGridPreset = (
  ...
  const matchesSummaryHeight =
    ...
    (card.kind === "summary"
      ? dimensions.width === "large" && dimensions.height === "large"
...
export const orderDashboardCardsByLayout = (
  cards: DashboardCardDefinition[],
  layout: DashboardGridItem[] | undefined,
) => {
  if (!layout?.length) {
    return cards;
  }
  ...
  const ordered = [...layout]
    ...
    .sort(
      (a, b) =>
        a.y - b.y ||
        a.x - b.x ||
        (cardIndexById.get(a.i) ?? 0) - (cardIndexById.get(b.i) ?? 0),
    )
    .flatMap((item) => {
...
const findDashboardMasonryPosition = (
  item: DashboardGridItem,
...
const getDashboardColumnHeights = (
  ...
  layout.forEach((item) => {
    for (
      let column = item.x;
      column < Math.min(item.x + item.w, cols);
      column += 1
    ) {
...
export const packDashboardMasonryLayout = (
  ...
  sourceLayout?.forEach((item) => {
    ...
    if (card && !sourceItemById.has(card.id)) {
      sourceItemById.set(
        card.id,
        normalizeDashboardGridItem(item, breakpoint, card, columnCount),
      );
    }
  });
  ...
  orderedCards.forEach((card) => {
    ...
    const position = findDashboardMasonryPosition(
      item,
      columnHeights,
      columnCount,
    );
    ...
    for (
      let column = packedItem.x;
      column < packedItem.x + packedItem.w;
      column += 1
    ) {
...
export const reconcileDashboardGridLayout = (
  ...
  sourceLayout.forEach((item) => {
    ...
    const normalizedItem = normalizeDashboardGridItem(
      item,
      breakpoint,
      card,
      columnCount,
  ...
  const missingItems = cards.flatMap((card) => {
    ...
    for (
      let column = packedItem.x;
      column < Math.min(packedItem.x + packedItem.w, columnCount);
      column += 1
    ) {
      columnHeights[column] = packedItem.y + packedItem.h;
    }
...
const appendMissingModuleChartLayoutItems = (
  ...
  savedLayout.forEach((item) => {
    if (
      !isModuleChartCardId(item.i) ||
      cardIds.has(item.i) ||
      layoutIds.has(item.i)
    ) {
      return;
    }
...
const dashboardLayoutsHaveSameFrames = (
  ...
  return left.every((item) => {
    const expected = rightById.get(item.i);
...
const migrateChangedDefaultLayouts = (
  ...
  return {
    cards: migratedCards,
    layouts: createDashboardLayouts(cards, migratedCards),
  };
};
...
export const reconcileDashboardPreferences = (
  ...
  const defaults = buildDefaultDashboardPreferences(
    creatorId,
    cards,
    actions,
    now,
  ...
  const reconciledCards = preserveMissingModuleChartCards(
    currentCards,
    preferences.cards,
    cardIds,
  );
  ...
  const sourceLayouts = Object.fromEntries(
    dashboardBreakpoints.map((breakpoint) => [
      breakpoint,
      (preferences.visual.layouts[breakpoint] ?? []).filter((item) =>
        cardIds.has(item.i),
      ),
    ]),
  ) as DashboardPreferencesV1["visual"]["layouts"];
  ...
  const layouts = Object.fromEntries(
    dashboardBreakpoints.map((breakpoint) => [
      breakpoint,
      appendMissingModuleChartLayoutItems(
        currentLayouts[breakpoint],
        preferences.visual.layouts[breakpoint] ?? [],
        cardIds,
      ),
    ]),
  ) as DashboardPreferencesV1["visual"]["layouts"];
  const tableSortField =
    preferences.table.sort.field === "size"
      ? "width"
      : preferences.table.sort.field;
...
const migrateDashboardPreferencesInput = (value: unknown) => {
  ...
  const cards = Object.fromEntries(
    Object.entries(candidate.cards as Record<string, unknown>).map(
      ([cardId, preference]) => {
        if (!preference || typeof preference !== "object") {
          return [cardId, preference];
        }

        const cardPreference = preference as {
          height?: unknown;
          size?: unknown;
          visible?: unknown;
          width?: unknown;
...
export const parseDashboardPreferences = (value: unknown) => {
  const parsed = DashboardPreferencesV1Schema.safeParse(
    migrateDashboardPreferencesInput(value),
  );
...
export const setDashboardSelectedView = (
  preferences: DashboardPreferencesV1,

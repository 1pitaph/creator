...
export const buildMetricCards = (
  ...
  const metricCards: MetricDefinition[] = [
    {
      ...
      askTarget: {
        ...
        evidence: diagnosis.metrics.history
          .map((item) => `${item.date}: ${formatCompact(item.views)}`)
          .slice(-3),
      },
    },
    {
      ...
      askTarget: {
        ...
        evidence: topContent
          ? [`高表现内容：${topContent.title}`, `Hook：${topContent.hook}`]
          : [],
      },
    },
    {
      ...
      askTarget: {
        ...
        evidence: diagnosis.creator.audience.map(
  ...
  if (
    typeof metrics.liveGmv7d === "number" ||
    typeof metrics.commerceConversionRate === "number"
  ) {
    metricCards.push({
      id: "commerce",
      label:
        diagnosis.creator.creatorType === "short_drama_strategy"
          ? "短剧收益"
          : "商业化承接",
      value:
        typeof metrics.liveGmv7d === "number"
          ? formatCurrency(metrics.liveGmv7d)
          : formatPct(metrics.commerceConversionRate ?? 0),
      ...
      chartIntent: {
        ...
        metricKeys:
          typeof metrics.liveGmv7d === "number"
            ? ["liveGmv", "commerceConversionRate"]
            : ["commerceConversionRate"],
        unit: typeof metrics.liveGmv7d === "number" ? "mixed" : "percent",
        timeRangeDays: 7,
      },
      askTarget: {
        title:
...
export const buildTrendComparisonChart = (): ChartIntent => ({
  ...
  title: "7 日指标冷热矩阵",
  description: "播放、完播、互动和转粉的 7 日冷热变化",
  metricKeys: [
    "views",
    "completionRate",
    "interactionRate",
...
export const buildDashboardViewModel = (
  ...
  topInsight:
    diagnosis.insights.find((insight) => insight.severity === "warning") ??
